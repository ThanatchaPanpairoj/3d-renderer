...
public class FaceDistanceComparator implements Comparator<Face>
{
    ...
    public int compare(Face f1, Face f2) {
        ...
        return (int)Math.sqrt(Math.pow(p2.getX(), 2) + Math.pow(p2.getY(), 2) + Math.pow(p2.getZ() + 2 * WIDTH, 2))
        - (int)Math.sqrt(Math.pow(p1.getX(), 2) + Math.pow(p1.getY(), 2) + Math.pow(p1.getZ() + 2 * WIDTH, 2));
    }
}
